...
class ConditionalUnet1D(nn.Module):
    def __init__(self, 
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out): # enumerate 取 index 和 (in_dim, out_dim)
            is_last = ind >= (len(in_out) - 1) # 判断当前是不是最后一层（最后一层不再做 downsample）
    ...
    def forward(self, 
        ...
        x = sample
        h = []
        for idx, (resnet, resnet2, downsample) in enumerate(self.down_modules): # enumerate 取 index 和 (in_dim, out_dim)
